...
if __name__ == "__main__":
    ...
    argparser.add_argument("--candidates-path", type=pathlib.Path, default=_DEFAULT_CANDIDATE_PATH, help="Chemin vers le fichier des candidats")
    
    argparser.add_argument("--poll-type", "-t", type=str, default="pt3", help="ID du type de sondage (par défaut : pt3)")
    
    argparser.add_argument("--prefix", "-x", type=str, default="ifop", help="Préfixe pour le fichier de sortie (par défaut : ifop)")
    argparser.add_argument("--population", "-P", type=str, default="all", help="Population cible (par défaut : all)")
    ...
    argparser.add_argument("--overwrite", "-o", action="store_true", help="Écraser le fichier de sortie s'il existe")
    ...
    if not args.overwrite and output_path.exists():
        print(f"Le fichier de sortie {output_path} existe déjà. Veuillez le supprimer, choisir un autre suffixe ou utiliser l'option --overwrite.")
        exit(1)
